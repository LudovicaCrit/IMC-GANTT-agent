...
- `models`: `Progetto`, `Task`, `Consuntivo`, `get_session` (lettura diretta Postgres).
...
from models import Utente, Progetto, Task, Consuntivo, get_session
...
def economia_margini(_: Utente = Depends(require_manager)):
    ...
    session = get_session()
    # P010 (Attività Interne) escluso: no valore contrattuale
    progetti = session.query(Progetto).filter(Progetto.id != "P010").all()
    result = []
    for p in progetti:
        cons_proj = session.query(Consuntivo).join(
