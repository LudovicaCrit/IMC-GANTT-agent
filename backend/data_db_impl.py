...
def tipo_unita_per_task(session, task_ids):
    ...
    stesso conflitto che il vecchio motore a cursore risolveva fra ore derivate
    dall'avanzamento e ore scritte a mano (le derivate vincevano): lì la regola
    stava in un punto solo, e qui deve stare in un punto solo. Dal passo 5.4 le
    ore derivate non esistono più — si dichiarano per giorno — ma la mutua
    esclusione serve ancora, perché `_valida_unita_blocchi` la usa per rifiutare
    le ore messe sul TASK di un task scomposto (M8/M9). Un task con pezzi che si porti
    ...
    `_motivo_non_modificabile` rifiuta di scriverci sopra. Un task i cui
...
def _lunedi(d=None):
    ...
    (salva_blocchi_settimana): la colonna `settimana` di Consuntivo e
    ConsuntivoSottotask contiene SEMPRE un lunedì. Se la scrittura ci mette un giorno
...
def task_settimana_dipendente(dipendente_id, settimana=None):
    ...
    try:
        ...
        #
        # Gli ANNULLATI restano fuori dalla lista: il salvataggio rifiuta con
        # 400 le ore su un pezzo annullato, e offrirlo compilabile nella griglia
        # vorrebbe dire una riga che al salvataggio esplode. I SOSPESI invece
...
def aggiungi_task(progetto_id, nome, fase, ore_stimate, data_inizio, data_fine,
    ...
    # chiamata anche dalla propagazione di `salva_blocchi_settimana`, dove a
    # scrivere è il DIPENDENTE che dichiara «ci sto lavorando». Bloccare lì punirebbe
...
# ══════════════════════════════════════════════════════════════════════
# L'UNICA scrittura delle ore. Nato accanto a `salva_consuntivo`, il motore a
# cursore che derivava le ore dall'avanzamento dichiarato: quello è uscito col
# passo 5.4 (23/09/2026), e con lui il concetto stesso di ora derivata. Qui le
# ore non si derivano da nulla, sono blocchi dichiarati per giorno.
